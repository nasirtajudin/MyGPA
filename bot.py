...
logging.basicConfig(
    ...
    level=logging.INFO,
    # Uncomment below to also write logs to a file:
    # handlers=[logging.StreamHandler(), logging.FileHandler("bot.log")],
)
# Reduce noise from PTB internals
...
# Each semester maps to a list of subjects with credit hours.
...
# Grading scale: (min_score, max_score, letter, grade_point)
...
# Keyed by Telegram user-id; cleared on restart.
user_sessions: dict[int, dict] = {}


def create_session() -> dict:
    """Create a fresh session dictionary for a user."""
    return {
        "first_gpa":     None,   # float | None
        "first_subjects": None,  # list[dict] | None  (subject breakdown)
        "second_gpa":     None,
        "second_subjects": None,
        "state":          "IDLE",  # conversation state string
        "temp":           {},      # ephemeral data during multi-step flows
    }
...
def reset_session(user_id: int) -> dict:
    """Wipe and recreate the session (used on /start or Main Menu)."""
    user_sessions[user_id] = create_session()
    return user_sessions[user_id]


# ─── Calculation Functions (pure, no Telegram dependency) ─────────────────────

def get_grade(score: float) -> tuple[str, float]:
    """
    Convert a numeric score into a letter grade and grade point.
    Returns (letter, point).
    """
...
def calculate_gpa(scores: list[float], subjects: list[dict]) -> dict:
    """
    Calculate semester GPA from a list of scores and subject metadata.

    Returns a dict with:
      gpa, subjects (detailed), total_credits, total_quality_points,
      highest (subject dict), lowest (subject dict)
    """
    total_quality = 0.0
    total_credits = 0
    ...
    # Highest = best grade point (tie-break: higher raw score)
    highest = max(subject_results, key=lambda s: (s["point"], s["score"]))
    # Lowest  = worst grade point (tie-break: lower raw score)
...
def calculate_cgpa(first_gpa: float, second_gpa: float) -> float:
    """CGPA = simple average of the two semester GPAs."""
...
def calculate_target_gpa(
    ...
    """
    Given a current CGPA and a target, compute the GPA needed in the
    *next* semester to reach the target.

    Returns None if the target is mathematically unreachable
    (would require GPA > 4.0).
    """
...
def get_academic_status(gpa: float) -> str:
    """Map a GPA value to a qualitative status label."""
    if gpa >= 3.75:
        return "Excellent"
    elif gpa >= 3.00:
        return "Very Good"
...
def get_class_of_degree(cgpa: float) -> str:
    """Predict the class of degree from CGPA."""
    if cgpa >= 3.75:
...
def get_placement_status(score: float) -> str:
    """Qualitative label for the final placement score."""
    if score >= 85:
...
def build_grade_distribution(subjects: list[dict]) -> str:
    """Build a small text chart showing how many of each grade were earned."""
    counts: dict[str, int] = {}
    for s in subjects:
...
def parse_scores(
    ...
    """
    Parse and validate comma-separated scores.

    Returns (scores_list, error_type).
    ...
    # Detect space-separated input (no commas)
...
def get_score_error_message(error_type: str) -> str:
    """Human-readable error message for score parsing failures."""
    return {
        "empty": (
            "Invalid input.\n\n"
...
def validate_number(
    ...
    """Validate a single numeric input within [min_val, max_val]."""
    text = text.strip()
    if not text:
        return None, "empty"
    try:
...
def get_number_error_message(
    ...
    """Human-readable error for single-number validation failures."""
    return {
...
def fmt(n: float) -> str:
    """
    Format a float to show full precision without scientific notation
    and without unnecessary trailing zeros.

    e.g. 3.746875 -> "3.746875",  15.0 -> "15",  0.0 -> "0"
    """
...
def gpa_result_kb(semester: str) -> InlineKeyboardMarkup:
    """Keyboard shown after a GPA calculation."""
...
def gpa_result_from_placement_kb() -> InlineKeyboardMarkup:
    """Keyboard shown after GPA calc that originated from the placement flow."""
    return InlineKeyboardMarkup([
...
def placement_options_kb(both_exist: bool) -> InlineKeyboardMarkup:
    """Keyboard for the placement-score entry-point menu."""
    buttons: list[list[InlineKeyboardButton]] = []
    if both_exist:
        buttons.append([
    ...
    buttons.append([
        InlineKeyboardButton("✏️ Enter GPA Manually",
...
def placement_missing_kb(session: dict) -> InlineKeyboardMarkup:
    """Keyboard for choosing which missing semester to calculate."""
    buttons: list[list[InlineKeyboardButton]] = []
    ...
    buttons.append([
        InlineKeyboardButton("⬅️ Back", callback_data="menu_placement")
    ])
    return InlineKeyboardMarkup(buttons)
...
def build_gpa_result_message(semester: str, result: dict) -> str:
    """Compose the full GPA result message with breakdown."""
...
def build_placement_result_message(result: dict) -> str:
    """Compose the full placement-score result message."""
...
def build_dashboard_message(session: dict) -> str:
    """Compose the academic dashboard with all available insights."""
    ...
    # ── CGPA & comparison (only if both exist) ──
    if has_first and has_second:
        ...
        # Suggestion: what's needed to reach the next quarter-point
...
def build_score_prompt(semester: str) -> str:
    """Build the 'enter your scores' prompt for a semester."""
...
async def safe_edit(query, text: str, reply_markup=None) -> None:
    """
    Edit the callback-query message; fall back to a new reply if editing
    fails (e.g. 'message not modified' or message was deleted).
    """
    try:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Central router for all inline-button presses."""
    ...
    # Menu-navigation callbacks always reset the conversation state.
...
async def start_gpa_calculation(query, session: dict, semester: str) -> None:
    """Show the score-entry prompt for the chosen semester."""
...
async def show_placement_menu(query, session: dict) -> None:
    """Placement-score entry point — adapts to what data already exists."""
    has_first = session["first_gpa"] is not None
    has_second = session["second_gpa"] is not None
...
async def placement_use_saved(query, session: dict) -> None:
    """Skip GPA entry — use already-calculated values."""
...
async def placement_manual_start(query, session: dict) -> None:
    """Begin manual GPA entry — ask for first semester GPA."""
...
async def placement_calculate_missing(query, session: dict) -> None:
    """Show which semester(s) are missing and let the user calculate them."""
    has_first = session["first_gpa"] is not None
    has_second = session["second_gpa"] is not None

    # Remember that we came from the placement flow.
...
async def show_target_calculator(query, session: dict) -> None:
    """Target CGPA Calculator — what GPA do you need next semester?"""
    has_first = session["first_gpa"] is not None
    has_second = session["second_gpa"] is not None
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Dispatch free-text input based on the user's current conversation state."""
    user_id = update.effective_user.id
    session = get_session(user_id)
    ...
    # ── Manual GPA entry (placement flow) ──
...
async def process_gpa_scores(
    ...
    """Parse scores, calculate GPA, and show the result."""
    subjects = SEMESTER_SUBJECTS[semester]
    ...
    # If this calc originated from the placement flow, offer a
    # "Continue to Placement" button instead of the normal result keyboard.
...
async def process_manual_gpa(
    ...
    """Validate a manually-entered GPA and advance the placement flow."""
...
async def process_entrance_score(
    ...
    """Validate entrance exam score (0–600) and ask for placement exam."""
...
async def process_placement_exam_score(
    ...
    """Validate placement exam score (0–30) and compute the final result."""
    value, error = validate_number(text, 0, 30)
    if error:
        await update.message.reply_text(get_number_error_message(error, 0, 30))
        return

    # Retrieve GPAs from temp (manual or saved) falling back to session.
    ...
    # Persist GPAs back to session if they weren't already saved.
    if session["first_gpa"] is None:
...
async def process_target_cgpa(
    ...
    """Validate target CGPA and show what's needed next semester."""
    ...
    if target <= cgpa + 0.0001:
        ...
        if needed is None:
            ...
            if needed >= 3.75:
                lines.append("⚠️ This is a very challenging target!")
...
def main() -> None:
    ...
    app.add_handler(CommandHandler("help", start_command))  # alias
